...
export default function ActivitiesService() {
  ...
  useEffect(() => {
    reset({
      has_activities_enabled: hotelData?.payload?.has_activities_enabled,
    });
  }, [hotelData, reset]);

  const onSubmit = async (data: { has_activities_enabled: boolean }) => {
    setError(undefined);
    try {
      const submitData = {
        ...data,
      ...
      <Row>
        <Column>
          <Card
            ...
            page
            header={<Typography h2>{t("Activities")}</Typography>}
          >
            <Row>
              <Column offset={3} size={6}>
                <Typography p muted>
                  {t("hotel_editor.activities_app.description")}
                </Typography>
                  ...
                  <Row>
                    <div className="ui-flex column">
                      <FormControl className="my-10">
                        <Toggle
                          data-testid="hotel-editor-activities-app-enabled"
                          label={t("Enabled")}
                          {...register("has_activities_enabled")}
                        />
                      </FormControl>
                    </div>
                    <div className="mb-30">
