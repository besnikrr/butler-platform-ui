...
const NavBar: React.FC = () => {
  const { tenant: { cognito }, setIsUserAuthenticated } = useContext(AppContext);
  const history = useHistory();

  const logOutOnClick = async () => {
    await SignOut(cognito);
    setIsUserAuthenticated && setIsUserAuthenticated(false);
  ...
  return (
    <div className='ui-flex between pt-10 pl-20'>
        <div className='network-navbar-header ui-flex v-center'>
          <div className="mr-40" onClick={() => history.push('/')}>
        ...
        <div className='ui-flex v-center mr-20'>
          <Dropdown
            renderTrigger={(openDropdown, triggerRef, open) => (
              <Button ref={triggerRef} onClick={openDropdown} iconOnly  style={{ background: 'none', padding: 0}}>
                <ProfilePlaceholder />
              </Button>
            )}
            placement='right'
          ...
          >
            ...
            >
              <Typography size={'large'} onClick={logOutOnClick}>Log out</Typography>
            </Button>
          </Dropdown>
        </div>
      </div>
